...
def upgrade() -> None:
    ...
    CPP_RULES = [
        ...
        {"id": "cpp-memory-002", "languages": ["c", "cpp"], "message": "[The Loop] Use-after-free — pointer used after deletion", "severity": "ERROR", "patterns": [{"pattern-regex": "delete.*\\n.*\\w+\\s*->"}], "metadata": {"category": "memory-safety", "cwe": "CWE-416"}},
        ...
        {"id": "cpp-error-001", "languages": ["c", "cpp"], "message": "[The Loop] Unchecked return value from system function — check errno", "severity": "WARNING", "patterns": [{"pattern-regex": "(open|read|write|recv|send)\\([^)]*\\)\\s*[;)]"}], "metadata": {"category": "error-handling"}},
