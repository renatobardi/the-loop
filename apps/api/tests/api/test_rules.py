...
async def test_get_latest_rules_from_cache(client: AsyncClient, mock_cache: AsyncMock) -> None:
    ...
    mock_cache.get_latest.assert_called_once()
...
async def test_get_latest_rules_from_service(
    ...
    mock_cache.get_latest.assert_called_once()
    mock_service.get_latest_active.assert_called_once()
    mock_cache.set_latest.assert_called_once()


async def test_get_latest_rules_not_found(client: AsyncClient, mock_cache: AsyncMock) -> None:
...
async def test_get_rules_by_version(client: AsyncClient, mock_service: AsyncMock) -> None:
    ...
    data = response.json()
    assert data["version"] == "0.1.0"
    mock_service.get_by_version.assert_called_once_with("0.1.0")


async def test_get_rules_by_version_not_found(client: AsyncClient, mock_service: AsyncMock) -> None:
...
async def test_list_all_versions(client: AsyncClient, mock_service: AsyncMock) -> None:
    ...
    assert data["versions"][1]["version"] == "0.1.0"
    assert data["versions"][1]["status"] == "deprecated"
...
async def test_publish_rules_success(
    ...
    mock_service.publish_version.assert_called_once()
    mock_cache.invalidate.assert_called_once()


async def test_publish_rules_version_exists(client: AsyncClient, mock_service: AsyncMock) -> None:
