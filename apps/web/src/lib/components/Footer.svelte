<script lang="ts">
  ...
  import { hero_branding, hero_product_name, footer_copyright, footer_constitution, footer_contact } from '$lib/paraglide/messages.js';
</script>

<footer class="bg-bg-surface border-t border-border py-12">
  <Container class="flex flex-col md:flex-row items-center md:items-start justify-between gap-8">
    <div class="text-center md:text-left">
      <p class="text-lg font-bold text-text">{hero_product_name()}</p>
      <p class="text-sm text-text-subtle">{hero_branding()}</p>
    </div>
