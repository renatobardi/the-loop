	...
	<section>
		...
		<dl class="grid gap-3 text-sm md:grid-cols-2">
			...
			{#if incident.organization}
				<div>
					<dt class="text-text-muted">Organization</dt><dd class="text-text">{incident.organization}</dd>
				</div>
			{/if}
			{#if incident.source_url}
				<div class="md:col-span-2">
					<dt class="text-text-muted">Source URL</dt><dd><a
						href={incident.source_url}
						target="_blank"
						rel="noopener"
						class="text-accent hover:underline">{incident.source_url}</a
					></dd>
				</div>
			{/if}
			{#if incident.failure_mode}
				<div class="md:col-span-2">
					<dt class="text-text-muted">Failure Mode</dt><dd class="text-text">{incident.failure_mode}</dd>
				</div>
			{/if}
		</dl>
	</section>
	...
	<section>
		...
		<dl class="grid gap-3 text-sm md:grid-cols-2">
			{#if incident.subcategory}
				<div>
					<dt class="text-text-muted">Subcategory</dt><dd class="text-text">{incident.subcategory}</dd>
				</div>
			{/if}
			{#if incident.affected_languages.length > 0}
				<div>
					<dt class="text-text-muted">Affected Languages</dt><dd class="flex gap-1">{#each incident.affected_languages as lang (lang)}<Badge>{lang}</Badge>{/each}</dd>
				</div>
			{/if}
			{#if incident.tags.length > 0}
				<div>
					<dt class="text-text-muted">Tags</dt><dd class="flex flex-wrap gap-1">{#each incident.tags as tag (tag)}<span
							class="rounded bg-bg-elevated px-1.5 py-0.5 text-xs text-text-muted">{tag}</span>{/each}</dd>
				</div>
			{/if}
		</dl>
	</section>
		...
		<div class="space-y-4 text-sm">
			...
			{#if incident.code_example}
				<div>
					...
					<pre
						class="mt-1 overflow-x-auto rounded-md bg-bg-elevated p-3 font-mono text-xs text-text">{incident.code_example}</pre>
				</div>
			{/if}
			<div>
			...
			{#if incident.semgrep_rule_id}
				<div>
					<span class="text-text-muted">Semgrep Rule ID:</span>
					<code class="rounded bg-bg-elevated px-1 text-xs">{incident.semgrep_rule_id}</code>
				</div>
			{/if}
		</div>
	</section>
